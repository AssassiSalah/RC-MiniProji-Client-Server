...
public class VirusFileChecker {

    private static final String SAVE_FILE_PATH = System.getProperty("user.home") + "/.last_opened_path"; // Hidden file in user home
    private boolean useVirusTotalAPI = false;
    public boolean isAllFilesSafe() {
		return allFilesSafe;
	}

	private boolean allFilesSafe = true;
    ...
    // Constructor for initializing last opened file path
    public VirusFileChecker() {
        this.lastOpenedFile = loadLastOpenedFile();
    }

    // Run the file check process
    public void runFileCheck() {
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            ...
            try (ProgressBar progressBar = new ProgressBar("Scanning Files", totalFiles.get())) {
                // Check the operating system and print it to the console
                ...
                for (int i = 0; i < allFiles.size(); i++) {
                    ...
                    // Check if the operating system is Windows or Ubuntu (Linux-based)
                    if (os.contains("Windows")) {
                        ...
                        // Assuming Ubuntu or any Linux-based OS, use VirusTotal API
                        try {
                        ...
                        System.out.println("Unknown Operating System, unable to scan.");
                        allFilesSafe = false;
                    }
    ...
    private File loadLastOpenedFile() {
    ...
    private void saveLastOpenedFile(File file) {
    ...
    public void checkFileWithVirusTotal(String filePath) throws IOException {
    ...
    private String extractFileHashFromResponse(String response) {
    ...
    private void getScanResult(String fileHash, String apiKey) throws IOException {
    ...
    // Getter for the status of all files' safety
    public boolean areAllFilesSafe() {
        return allFilesSafe;
    }
}
