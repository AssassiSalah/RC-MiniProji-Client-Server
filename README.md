...
> 📄 **For detailed theoretical documentation**, see the full project report: [MiniProjectRc.pdf](Report/MiniProjectRc.pdf)
...
- **Key Size**: 256-bit symmetric key for strong encryption
- **Mode**: CBC (Cipher Block Chaining) for secure block encryption
- **Padding**: PKCS#5 for handling variable-length data
- **Use**: Encrypts all file transfers between client and server

#### 2. RSA-2048 (Rivest-Shamir-Adleman)
- **Key Size**: 2048-bit asymmetric key pair
- **Use**: Secure exchange of AES session key and IV
- **Process**: Server's public key encrypts the AES key; private key decrypts it

#### 3. SHA-256 (Secure Hash Algorithm)
- **Use**: Password hashing and file integrity verification
- **Benefit**: Detects any tampering or corruption during transfer
...
└──────┬──────┘                              └──────┬──────┘
       ...
       │◄───────────────────────────────────────────│
       │                                            │
       │  3. Generate AES-256 Key + IV              │
       │                                            │
       │  4. Encrypt AES Key & IV with RSA          │
       │                                            │
       │  5. Send Encrypted AES Key & IV            │
       │───────────────────────────────────────────►│
       │                                            │
       │  6. Decrypt AES Key & IV with RSA Private  │
       │                                            │
...
2. Each chunk is encrypted using AES-256-CBC
3. SHA-256 hash calculated during encryption
4. Encrypted data + hash sent to server
5. Server decrypts and verifies hash integrity
6. File scanned for viruses before saving

#### Download Process (Server → Client)
1. Server reads requested file in chunks
2. Each chunk encrypted using session AES key
3. SHA-256 hash calculated for integrity
4. Encrypted data + hash sent to client
5. Client decrypts and verifies hash
6. File saved only if integrity check passes

### Why AES-CBC?

| Feature | Benefit |
|---------|---------|
| **Chaining Dependency** | Identical blocks produce different ciphertext |
| **IV Randomization** | Same file encrypted twice produces different output |
| **Performance** | Fast encryption/decryption for large files |
| **Compatibility** | Works with PKCS#5 padding for any file size |

### File Integrity Verification

- Every file transfer includes SHA-256 hash verification
- Files are scanned for viruses after upload (ClamAV/Defender/VirusTotal)
- Corrupted or tampered transfers are automatically rejected
...
> 📄 **للاطلاع على الوثائق النظرية التفصيلية**، راجع تقرير المشروع الكامل: [MiniProjectRc.pdf](Report/MiniProjectRc.pdf)
...
#### 1. AES-256 (معيار التشفير المتقدم)
- **حجم المفتاح**: مفتاح متماثل 256 بت للتشفير القوي
- **الوضع**: CBC (تسلسل كتل التشفير) للتشفير الآمن
- **الحشو**: PKCS#5 للتعامل مع البيانات متغيرة الطول
- **الاستخدام**: يشفر جميع عمليات نقل الملفات بين العميل والخادم

#### 2. RSA-2048 (ريفست-شامير-أدلمان)
- **حجم المفتاح**: زوج مفاتيح غير متماثل 2048 بت
- **الاستخدام**: تبادل آمن لمفتاح جلسة AES و IV
- **العملية**: المفتاح العام للخادم يشفر مفتاح AES؛ المفتاح الخاص يفك التشفير

#### 3. SHA-256 (خوارزمية التجزئة الآمنة)
- **الاستخدام**: تجزئة كلمات المرور والتحقق من سلامة الملفات
- **الفائدة**: يكتشف أي تلاعب أو تلف أثناء النقل

### سير عمل تبادل المفاتيح
...
└──────┬──────┘                              └──────┬──────┘
       ...
       │───────────────────────────────────────────►│
       │                                            │
       │  2. إرسال المفتاح العام RSA                │
       │◄───────────────────────────────────────────│
       │                                            │
       │  3. توليد مفتاح AES-256 + IV               │
       │                                            │
       │  4. تشفير مفتاح AES و IV باستخدام RSA      │
       │                                            │
       │  5. إرسال مفتاح AES و IV المشفرين          │
       │───────────────────────────────────────────►│
       │                                            │
       │  6. فك تشفير مفتاح AES و IV بالمفتاح الخاص │
       │                                            │
       │  7. تم تأسيس جلسة آمنة                     │
       │◄──────────────────────────────────────────►│
...
#### عملية الرفع (العميل ← الخادم)
1. العميل يقرأ الملف على شكل قطع
2. كل قطعة تُشفر باستخدام AES-256-CBC
3. حساب تجزئة SHA-256 أثناء التشفير
4. إرسال البيانات المشفرة + التجزئة إلى الخادم
5. الخادم يفك التشفير ويتحقق من سلامة التجزئة
6. فحص الملف بحثاً عن الفيروسات قبل الحفظ

#### عملية التحميل (الخادم ← العميل)
1. الخادم يقرأ الملف المطلوب على شكل قطع
2. كل قطعة تُشفر باستخدام مفتاح جلسة AES
3. حساب تجزئة SHA-256 للسلامة
4. إرسال البيانات المشفرة + التجزئة إلى العميل
5. العميل يفك التشفير ويتحقق من التجزئة
6. حفظ الملف فقط إذا نجح فحص السلامة

### لماذا AES-CBC؟

| الميزة | الفائدة |
|--------|---------|
| **الاعتماد المتسلسل** | الكتل المتطابقة تنتج نصوصاً مشفرة مختلفة |
| **عشوائية IV** | نفس الملف المشفر مرتين ينتج مخرجات مختلفة |
| **الأداء** | تشفير/فك تشفير سريع للملفات الكبيرة |
| **التوافق** | يعمل مع حشو PKCS#5 لأي حجم ملف |

### التحقق من سلامة الملفات

- كل عملية نقل ملفات تتضمن التحقق من تجزئة SHA-256
- يتم فحص الملفات بحثاً عن الفيروسات بعد الرفع (ClamAV/Defender/VirusTotal)
- يتم رفض عمليات النقل التالفة أو المُتلاعب بها تلقائياً
